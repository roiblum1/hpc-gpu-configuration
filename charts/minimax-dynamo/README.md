# minimax-dynamo — Phase 6 (env/h100-8x-ib)

MiniMax M2.7 + **DFlash** speculative decoding on **8× DGX H100 over InfiniBand**, served as one `DynamoGraphDeployment`: a KV-router Frontend plus **4 wide-EP worker gangs of 2 nodes each** (Grove `multinode`). This chart is the Dynamo + Grove translation of the LeaderWorkerSet design in [minimax-m27-dflash-design.md](../../minimax-m27-dflash-design.md) (original manifest kept at [reference/minimax-m27-dflash-lws.yaml](../../reference/minimax-m27-dflash-lws.yaml)).

## The topology in one paragraph

M2.7 (230B total / 10B active MoE, 256 experts) fits on one 8×H100 node — multi-node is a *throughput optimization*, not a requirement. Inside each node: TP8 on NVLink. Across the 2 nodes of a gang: DP2 + `--enable-expert-parallel` ⇒ experts sharded **EP16-wide**, MoE all-to-all over IB via **DeepEP**; attention/KV stay node-local. Sharding experts 16-wide frees ~14–15 GB HBM per GPU, which becomes KV cache ⇒ more concurrent sequences at daytime peak. A node loss kills exactly **one** gang; the other 3 serve at 75%. Escape hatch (design §1): if spec-decode + multi-node DP+EP misbehave on your vLLM build, set `worker.replicas: 8`, `nodeCount: 1`, `dataParallel: 1` — 8 single-node islands, zero re-architecture.
...
- **One DGD** — `Frontend` (KV router, infra nodes) + `MinimaxWorker` (4 gangs × 2 nodes, full-node pods: 8 GPUs + 8 IB rail VFs + integer CPUs).
...
helm template minimax-dynamo --set speculative.method=mtp # zero-artifact fallback rendering
```
...
|-------|---------|--------------|
| `worker.replicas` / `nodeCount` | `4` / `2` | 4 gangs × 2 nodes = all 8 DGX H100; each gang gang-scheduled by KAI |
| `worker.tensorParallel` / `dataParallel` | `8` / `2` | TP8 on NVLink per node; DP2 + EP ⇒ EP16 across the gang over IB |
| `speculative.method` | `dflash` | **DFlash by default** (draft: `z-lab/MiniMax-M2.7-DFlash`, staged). Flip to `mtp` for the zero-artifact fallback |
| `speculative.numSpeculativeTokens` | `8` | High K is right for DFlash — all K tokens cost one draft forward |
...
Validation ladder first (design §7): single node TP8 no spec → single node + MTP → 2-node gang DP+EP no spec → full config. Then: `genai-perf` meets TTFT/ITL · DeepEP all-to-all clean under load (dispatch p99 on the dashboard) · spec-decode acceptance ≥ threshold at night-shape traffic, auto-disables at peak-shape · node kill: exactly one gang dies, service continues at 75%, gang rejoins when the node returns · `numastat`: engine memory on both sockets.
