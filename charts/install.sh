...
PLAN=(
  ...
  # sriov-rails intentionally NOT in this plan: rail policies/NADs are user-provided
  # templated config, applied out of band. Gate 3 still MUST be run against those rails:
  #   Gate 3: ib_write_bw >=370Gb/s/rail; nccl all_reduce at ref; 2-node alltoall clean,
  #           zero PFC pause/out_of_buffer; NCCL_DEBUG=INFO log shows GDRDMA
  ...
  "glm51-dynamo|glm-serving|Gate 6: genai-perf meets TTFT/ITL per lane; KV router spreads sessions with prefix locality across both replicas; numastat: engine memory on both sockets (no KVBM/NIXL checks — aggregated shape)"
